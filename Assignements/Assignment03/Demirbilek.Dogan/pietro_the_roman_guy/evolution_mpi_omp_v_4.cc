...
#define CPU_NUM_THREAD_MAX 9 // number of cores allowed for one CPU
...
typedef struct {
	...
	double delta_t_max[1]{0};
	double delta_t[1]{0};
	double tmp_delta_t_max[1]{0};
	double evolution_time[1]{0};
	const int time_to_write_checkpoint {3}; // each 3 evolution of the system save a checkpoint file
} evolution_paramiter;
typedef struct {int size_array_group[4] {-1, -1, -1, -1}; int *array_group[4]{};} mpi_group_parameter;
...
int check_workers(const int& N_t_process, int&  N_t_thread, const int&  N_p, const int&  myid_process){
	...
	int N_t_thread_optimal {(N_p/N_t_process < CPU_NUM_THREAD_MAX) ? N_p/N_t_process : CPU_NUM_THREAD_MAX};

	if(N_t_process*N_t_thread > N_p && N_t_thread == 1){ // catch exceptions
		if(myid_process == REF_PROCESS) 
			std::cout << "------------------------------------------------------------\n"
								 << "------------------------------------------------------------\n\n"
								 << "ERROR: the dimension of the problem is less than the number of CPU, it means some CPU are unsed.\n" 
								 << "Set an appropriate number of \"workers\" for this problem.\n\n"
								 << "------------------------------------------------------------\n"
								 << "------------------------------------------------------------\n";
			return 1;
	} else if(N_t_process*N_t_thread > N_p && N_t_thread > CPU_NUM_THREAD_MAX && N_t_process*CPU_NUM_THREAD_MAX > N_p){
...
void build_mpi_data_type(MPI_Datatype& MPI_particle_type){
	//------- DEFINE NEW DATATYPE ------//
	const std::size_t  num_members {3};
	int blocklengths[num_members] = {3, 3, 1};
	
	MPI_Aint displacements[num_members] = {offsetof(particle, pos), offsetof(particle, vel), offsetof(particle, E)};
	MPI_Datatype types[num_members] = {MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE};
	
	MPI_Type_create_struct(num_members, blocklengths, displacements, types, &MPI_particle_type);
...
void deallocate_memories(MPI_Group* group, MPI_Datatype& MPI_particle_type, MPI_File& fp_1, MPI_File& fp_2,
												 const particle* array_all_particle_process, const particle* array_all_particle,
	...
	delete[] array_all_particle_process;
...
int main(int argc, char *argv[]){
	...
	// open file
	if(MPI_File_open(MPI_COMM_WORLD, "data_ic.txt", MPI_MODE_RDONLY, MPI_INFO_NULL, &fp_1) == 1){
		if(CHECK) if(myid_process == REF_PROCESS) std::cout << "ERROR: fp_1 is not pointing to the \"data_ic\" file.\n";
		return 1;
	}
	if(MPI_File_open(MPI_COMM_WORLD, "check_point.txt", MPI_MODE_CREATE|MPI_MODE_WRONLY, MPI_INFO_NULL, &fp_2) == 1){
	...
	MPI_File_read_at_all(fp_1, 4, tmp_N_p, 1, MPI_INTEGER, &status);
	int N_p {tmp_N_p[0]};
	...
	if(check_workers(N_t_process, N_t_thread, N_p, myid_process) == 1) {MPI_Finalize(); return 0;}
	
	// setting number of particles per thread
	int num_part_per_thread {num_part_per_process/N_t_thread};
	int R_thread {num_part_per_process%N_t_thread};
	
	//------- DEFINE NEW DATATYPE ------//
	build_mpi_data_type(MPI_particle_type);
	
	//--------------------------- BUILDING GROUPS ---------------------------//
	mpi_group_parameter group_magnitude{}; // default constructor
		
	if(build_group(N_t_process, myid_process, R_process, group, group_magnitude, comm) == 1) return 0;
	...
	#pragma omp parallel num_threads(N_t_thread) firstprivate(num_part_per_thread)// START PARALLEL REGION
	{	
		int myid_thread {omp_get_thread_num()};
		double potenzial_delta_t_max {0};
		
		if(myid_thread < R_thread) ++num_part_per_thread;
		...
		if(SUMMARY)
			if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) 
				 std::cout <<"The speaker is thread " << REF_THREAD << " in process " << REF_PROCESS << "\n"
									  ...
									  << "num_part_per_thread = " << num_part_per_thread << "\n"
									  << "start_particle_process = " << start_particle << ", end_particle_process = " << end_particle << "\n";
		
		if(CHECK)
			if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "	start iter loop\n";	
		...
		for(int iter = 0; iter < system.iter_max; ++iter){
			...
			system.tmp_delta_t_max[0] = 0;
			
			if(CHECK)
				if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "		start omp loop\n";	
			
			// customization of the omp reduction to apply it on the struct type
			#pragma omp declare reduction(max : evolution_paramiter : \
					omp_out.tmp_delta_t_max[0] = (omp_in.tmp_delta_t_max[0] > omp_out.tmp_delta_t_max[0]) ? omp_in.tmp_delta_t_max[0] : omp_out.tmp_delta_t_max[0]) \
					initializer(omp_priv(omp_orig))	
			
			#pragma omp for reduction(max : system) schedule(static)
			for(int i = start_particle; i < end_particle; ++i){ // makes for each particle that has to be considered
				...
				if(CHECK)
					if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "			start force 1 loop\n";
				...
				if(CHECK)
					if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "			end force 2 loop\n";	
				...
				potenzial_delta_t_max = system.epsilon*norm(array_all_particle_process[i - start_particle].vel)/norm(acceleration_i);
				if(system.tmp_delta_t_max[0] < potenzial_delta_t_max) system.tmp_delta_t_max[0] = potenzial_delta_t_max;
//				if(SUMMARY) if(myid_process == REF_PROCESS && myid_thread == REF_THREAD) std::cout << i << ": potenzial_delta_t_max = " << potenzial_delta_t_max << "\n";
			}	// END THREADS COMPUTATION PART
			
//			if(SUMMARY) if(myid_thread == REF_THREAD) std::cout  << myid_process << ": final thread potenzial_delta_t_max = " << system.tmp_delta_t_max[0] << "\n";
			
			if(CHECK)
				if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "		end omp loop\n";	
			
			// master threads determine the bigger delta t max from all the particles under the domain of the own process
			MPI_Allreduce(system.tmp_delta_t_max, system.delta_t_max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
			
	//		if(SUMMARY) if(myid_thread == REF_THREAD) std::cout  <<  myid_process << ": final mpi potenzial_delta_t_max = " << system.delta_t_max[0] << "\n";			
						
			#pragma omp master // defines next delta t and update the history time of the system
			{
				system.delta_t[0] = 0.5*system.delta_t_max[0];
				system.evolution_time[0] += system.delta_t[0];
			}
...
/*				if(myid_process == REF_PROCESS && myid_thread == REF_THREAD) std::cout << "array_all_particle_process[" << i - start_particle << "] = ";
				for(int j = 0; j < 3; ++j){
					if(myid_process == REF_PROCESS && myid_thread == REF_THREAD)   std::cout << array_all_particle_process[i - start_particle].pos.coord[j] << " ";
				}  if(myid_process == REF_PROCESS && myid_thread == REF_THREAD) std::cout << "\n";*/
				
				array_all_particle[i] = array_all_particle_process[i - start_particle];
			}
			
			if(CHECK)
				if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "		(after first barrier)\n";
			...
			if(CHECK)
				if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "		(after second barrier)\n";
			
			if(iter%system.time_to_write_checkpoint == 0){ // it's time to write a checkpoint file
				MPI_File_write_at_all(fp_2, 0, inode, 3, MPI_INT, &status);
				MPI_File_write_at_all(fp_2, 12, system.evolution_time, 1, MPI_DOUBLE, &status);
				MPI_File_write_at_all(fp_2, offset_process, array_all_particle, num_part_per_process, MPI_particle_type, &status);			
			}
		}
		
		if(CHECK)
			if(myid_thread == REF_THREAD && myid_process == REF_PROCESS) std::cout << "	end iter loop\n";	
	...
	//-------------------------- DEALLOCATE MEMORIES --------------------------//
	deallocate_memories(group, MPI_particle_type, fp_1, fp_2,
